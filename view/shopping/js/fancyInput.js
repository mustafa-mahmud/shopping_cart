...
(function ($) {
    ...
    var fancyInput = {
        ...
        clear: function (textCont) {
            var carets = $(textCont.parentNode).find('.carets');
            $(textCont).html(carets);
        },
        ...
        keydown: function (e) {
            var inputElm = e.target,
                    ...
                    caretsAtEndNoSelection = (inputElm.selectionEnd == inputElm.selectionStart && inputElm.selectionEnd == inputElm.value),
                    deleteKey = e.keyCode == 46 && !caretsAtEndNoSelection;
            ...
            if (e.keyCode == 8 || deleteKey) {
                ...
                // on pressing 'delete' while nothing is selected, and carets is not at the end
            ...
            // make sure to reset the container scrollLeft when carets is the the START or ar the END
        ...
        allEvents: function (e) {
            ...
            // The carets height should be set. only once after the first character was entered.
            if (!letterHeight) {
        ...
        setCaret: function (input) {
            var carets = $(input.parentNode).find('.carets'),
            ...
            if (pos == input.value) {
                //if( !chars.length )
                //	carets.prependTo( input.nextElementSibling );
                //else
                carets.appendTo(input.nextElementSibling);
            }
            else
                carets.insertBefore(insertPos);
        },
        getCaretPosition: function (input) {
            var caretsPos, direction = getSelectionDirection.direction || 'right';
            if (input.selectionStart || input.selectionStart == '0')
                caretsPos = direction == 'left' ? input.selectionStart : input.selectionEnd;
    ...
    function init(inputs) {
        ...
        inputs.each(function () {
            var className = 'fancyInput',
                    template = $('<div><b class="carets">&#8203;</b></div>');
